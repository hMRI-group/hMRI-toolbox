...
%-Check for existing extended header
%--------------------------------------------------------------------------
xhdr{1} = [];
if has_extended_header(V(1).fname)
    xhdr = get_metadata(V(1).fname);
end
...
if has_extended_header(V(1).fname)
    set_metadata(ni.dat.fname, xhdr{1}, struct('extended',true,'separate',true,'overwrite',true));
end
