...
isc        = cfg_menu;
isc.tag    = 'isc';
isc.name   = 'Imperfect spoiling correction';
isc.help   = {['The  RF- and gradient-spoiled gradient echo sequences used to ' ...
    'acquire the multiparametric mapping (MPM) data use RF and gradient spoiling to ' ...
    'destroy unwanted transverse magnetisation. Imperfect spoiling can leave ' ...
    'residual bias in the apparent R1 map if no further correction is used ' ...
    '[Preibisch and Deichmann 2009]. For specific MPM protocols using the customised ' ...
    'sequences, the hMRI-toolbox provides spoiling correction coefficients to ' ...
    'account for deviation from the Ernst equation. The coefficients are ' ...
    'sequence-specific and can be determined by simulation [Preibisch and Deichmann 2009].'], ...
    'The following options are available:', ...
    '- None: by default the correction is disabled. ', ...
    '- Standard: if your MPM protocol is one of the standard, ' ...
    'predefined ones, the correction parameters can be automaticallyselect the "Standard" option. ' ...
    'You can also provide the toolbox with a customized set of correction parametersSelect the "Customized" ](DefaultsAndCustomization). An additional module that efficiently calculates the protocol-specific correction parameters required to account for imperfect spoiling is planned.

    



The user can review and keep track of all the information ' ...
...
isc.labels = {'Disable' 'Enable'};
isc.values = {false true};
isc.val = {true};
