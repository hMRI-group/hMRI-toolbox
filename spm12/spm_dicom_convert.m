function out = spm_dicom_convert(Headers,opts,RootDirectory,format,OutputDirectory,json)
...
defaultjson = struct('extended',false,'separate',false,'anonym','basic');
if nargin<6 
    json = defaultjson; 
else
    % to solve conflicts with SPM version
    if ~isstruct(json)
        spmjson = json;
        clear json;
        json = defaultjson;
    ...
    if ~isfield(json,'extended');json.extended = defaultjson.extended;end
    if ~isfield(json,'separate');json.separate = defaultjson.separate;end
    if ~isfield(json,'anonym');json.anonym = defaultjson.anonym;end    
end
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic)
    fmos = ConvertMosaic(mosaic,RootDirectory,format,OutputDirectory,json);
end
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard)
    fstd = ConvertStandard(standard,RootDirectory,format,OutputDirectory,json);
end
if (strcmp(opts,'all') || strcmp(opts,'spect')) && ~isempty(spect)
    fspe = ConvertSpectroscopy(spect,RootDirectory,format,OutputDirectory,json);
end
if (strcmp(opts,'all') || strcmp(opts,'multiframe')) && ~isempty(multiframe)
    fmul = ConvertMultiframes(multiframe,RootDirectory,format,OutputDirectory,json);
end
...
%==========================================================================
% function fnames = ConvertMosaic(Headers,RootDirectory,format,OutputDirectory,json)
%==========================================================================
function fnames = ConvertMosaic(Headers,RootDirectory,format,OutputDirectory,json)
...
for i=1:length(Headers)
    ...
    if json.extended || json.separate
        Nii = init_metadata(Nii,Headers{i},json);
    end
...
%==========================================================================
% function fnames = ConvertStandard(Headers,RootDirectory,format,OutputDirectory,json)
%==========================================================================
function fnames = ConvertStandard(Headers,RootDirectory,format,OutputDirectory,json)
[Headers,dist] = SortIntoVolumes(Headers);
fnames         = cell(length(Headers),1);
for i=1:length(Headers)
    fnames{i} = WriteVolume(Headers{i},RootDirectory,format,OutputDirectory,dist{i},json);
end
...
%==========================================================================
% function fname = WriteVolume(Headers, RootDirectory, format, OutputDirectory, dist, json)
%==========================================================================
function fname = WriteVolume(Headers, RootDirectory, format, OutputDirectory, dist, json)
...
% for JSON metadata
if json.extended || json.separate
    Nii = init_metadata(Nii,Headers{1},json);
end
...
%==========================================================================
% function fnames = ConvertSpectroscopy(Headers, RootDirectory, format, OutputDirectory, json)
%==========================================================================
function fnames = ConvertSpectroscopy(Headers, RootDirectory, format, OutputDirectory, json)
fnames = cell(length(Headers),1);
for i=1:length(Headers)
    fnames{i} = WriteSpectroscopyVolume(Headers(i), RootDirectory, format, OutputDirectory, json);
end


%==========================================================================
% function fname = WriteSpectroscopyVolume(Headers, RootDirectory, format, OutputDirectory, json)
%==========================================================================
function fname = WriteSpectroscopyVolume(Headers, RootDirectory, format, OutputDirectory, json)
% Output filename
%-------------------------------------------------------------------
...
%==========================================================================
% function fspe = ConvertMultiframes(Headers, RootDirectory, format, OutputDirectory, json)
%==========================================================================
function fspe = ConvertMultiframes(Headers, RootDirectory, format, OutputDirectory, json)
fspe = {};
DicomDictionary = ReadDicomDictionary;
for i=1:numel(Headers)
    out  = ConvertMultiframe(Headers{i}, DicomDictionary, RootDirectory, format,OutputDirectory,json);
    fspe = [fspe(:); out(:)];
end


%==========================================================================
% function out = ConvertMultiframe(Header, DicomDictionary, RootDirectory, format, OutputDirectory, json)
%==========================================================================
function out = ConvertMultiframe(Header, DicomDictionary, RootDirectory, format, OutputDirectory, json)
out      = {};
...
for n=1:size(ord,2)
    ...
    for cNii = 1:size(pinfo,1)
        ...
        if json.extended || json.separate
            ...
            Nii{cNii} = init_metadata(Nii{cNii},cHeader{cNii},json);
        end
