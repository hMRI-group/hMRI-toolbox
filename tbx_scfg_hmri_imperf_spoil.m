...
% 
% PURPOSE: Compute correction factors for imperfect spoiling using the 
% method described in Corbin & Callaghan (MRM, 2021), which is an 
% extension of the method from Preibisch & Deichmann (MRM, 2009). 
%
...
% ---------------------------------------------------------------------
% Combine tissue and sequence parameters
% ---------------------------------------------------------------------
...
params        = cfg_choice;
params.tag    = 'params';
params.name   = 'Parameter switch (diffusion spoiling?)';
params.help   = {'Choose whether to include diffusion spoiling or not. ' ...
    'Including diffusion spoiling will give more accurate results, especially at 7T, but requires knowing more details of the sequence (Corbin & Callaghan, MRM, 2021). ' ...
    'In the absence of diffusion spoiling, imperfect spoiling correction reduces to the original method in Preibisch & Deichmann (MRM, 2009).'};
params.values = {params_diff params_nodiff};
params.val    = {params_diff};
...
imperf_spoil.val  = { outdir prot_name params small_angle_approx };
imperf_spoil.help = {'Given input info about the sequence settings and expected tissue properties, ' ...
    ...
    'using the method proposed by Preibisch & Deichmann, MRM 2009, 61(1):125 extended by '...
    'Corbin & Callaghan, MRM 2021, 86:693-708 to include gradient spoiling'};
