...
% Average first few echoes for increased SNR
...
for p=1:dm(3),
    ...
    if numel(Nmap)>4&&(TR_mtw == TR_pdw) && (fa_mtw == fa_pdw),
        ...
        % write MTR image
        Nmap(5).dat(:,:,p) = max(min(MTR,vbq_get_defaults('MTRthresh')),-vbq_get_defaults('MTRthresh'));
    ...
    if (vbq_get_defaults('PDmap'))%The two outer voxels in all directions are nulled in order to remove artefactual effects from the MT map
        MTforA=MT;
        ...
        tmp      = MTforA;
        if (TR_mtw == TR_pdw) && (fa_mtw == fa_pdw)
            Nmap(7).dat(:,:,p) = max(min(tmp,vbq_get_defaults('MTthresh')),-vbq_get_defaults('MTthresh'));
        else
            Nmap(5).dat(:,:,p) = max(min(tmp,vbq_get_defaults('MTthresh')),-vbq_get_defaults('MTthresh'));
        end
    end
