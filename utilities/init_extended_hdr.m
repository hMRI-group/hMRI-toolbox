...
% called by spm_dicom_convert (or hMRI_dicom_convert - tbc) and is applied
% to the nifti object being created. The DICOM header returned by
% spm_dicom_headers is tidied up and converted into JSON. The size of the
% JSON header is used to define a new offset (N.dat.offset) for the image
% data in the nifti file and the JSON header is written into the nifti
% file. The modified nifti object N is returned so the data can be written
% in it according to the new offset (in spm_dicom_convert).
...
%       Copyright (C) 2011-2015 Qianqian Fang <fangq at
%       nmr.mgh.harvard.edu> 
%       License: BSD or GNU General Public License version 3 (GPL v3), see
%       License*.txt 
...
% birth and DICOM filename (often containing the patient name) have been
% removed, while patient age (years), sex, size and weight are kept.   
...
if isfield(spm_tdyhdr,'Filename')
    spm_tdyhdr.Filename = 'AnonymousFileName';
end

% ORGANIZE STRUCTURE WITH acqpar AND history FIELDS
exthdr.history.procstep = struct('descrip','dicom to nifti import', 'version',hMRI_get_version, 'procpar',[]);
exthdr.history.input{1} = struct('filename','AnonymousFileName', 'history',[]);
if isfield(spm_tdyhdr,'ImageType')
    exthdr.history.output = struct('imtype',spm_tdyhdr.ImageType, 'units','a.u.');
else
    exthdr.history.output = struct('imtype','Unprocessed MR image', 'units','a.u.');
end
