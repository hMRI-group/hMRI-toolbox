function [parValue, outParName] = hmri_get_metadata_val(varargin)
...
% can therefore be used to efficiently search the metadata structure. 
%
% USAGE AND EXAMPLES
% [parValue, outParName] = hmri_get_metadata_val(mstruc, inParName)
% [parValue, outParName] = hmri_get_metadata_val(filenam, inParName)
% parValue = hmri_get_metadata_val(filenam, 'RepetitionTime')
%
% ARGUMENTS
...
%   not unique) giving the location of the retrived value(s) in the
%   metadata structure.
%
%--------------------------------------------------------------------------
...
if nargin~=2
    error('Wrong number of arguments. Type: help hmri_get_metadata_val for usage.');
end
...
if ischar(mstruc)
    mstruc = hmri_get_metadata(mstruc);
    mstruc = mstruc{1};
end
...
switch inParName
    ...
    case 'AllDiffusionDirections'
        if hMRI_get_extended_mstruc_val(mstruc,'isDWI')
    ...
    case 'epiReadoutDuration' % [ms]
        ...
        if nFieldFound
            ...
            valEPI = hMRI_get_extended_mstruc_val(mstruc, 'ScanningSequence');
            valSEQ = hMRI_get_extended_mstruc_val(mstruc, 'SequenceName');
            valPROT = hMRI_get_extended_mstruc_val(mstruc, 'ProtocolName');
            if strcmp(valEPI,'EP')
            ...
            elseif strfind(lower(valSEQ),'b1')
                ...
                measPElin = hMRI_get_extended_mstruc_val(mstruc,'MeasuredPELines');
                cRes = 1;
    ...
    case 'B1mapNominalFAValues' % [deg] for al_B1mapping - version dependent!!
        ...
        if nFieldFound
            ...
            if ~isempty(outParName)
                nmeas = hMRI_get_extended_mstruc_val(mstruc,'NumberOfMeasurements');
    ...
    otherwise
        ...
        [parValue,outParName] = get_val_nam_list(mstruc, nFieldFound, fieldList);
